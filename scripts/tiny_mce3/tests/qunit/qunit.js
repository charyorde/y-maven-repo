...
var QUnit = {
			...
			if ( tests ) {
				...
				addEvent(b, "dblclick", function(e) {
					...
					if ( window.location && target.nodeName.toLowerCase() === "strong" ) {
                        var newSearchParam = encodeURIComponent(getText([target]).replace(/\(.+\)$/, "").replace(/(^\s*|\s*$)/g, ""));
                        if(window.location.search.length > 0){
                            window.location.search += '&' + newSearchParam;
                        }else{
                            window.location.search = "?" + newSearchParam;
                        }
					}
	...
	equal: function(actual, expected, message) {
		return push(expected == actual, actual, expected, message);
	},
	...
	deepEqual: function(a, b, message) {
		return push(QUnit.equiv(a, b), a, b, message);
	},
