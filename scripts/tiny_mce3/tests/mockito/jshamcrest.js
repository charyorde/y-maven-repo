...
JsHamcrest = {
  ...
  areArraysEqual: function(array, anotherArray) {
    ...
    if (array instanceof Array || anotherArray instanceof Array) {
      ...
      for (var i = 0; i < array.length; i++) {
        ...
        if (!JsHamcrest.areObjectsEqual(a, b)) {
          return false;
        }
      }
  ...
  /**
   * Returns whether the given objects are equivalent.
   */
  areObjectsEqual: function(object, anotherObject) {
    if (object instanceof Array && anotherObject instanceof Array) {
      return JsHamcrest.areArraysEqual(object, anotherObject);

    } else if (typeof object == 'object' && typeof anotherObject == 'object' && object && anotherObject) {
      return equalProps(object, anotherObject) && equalProps(anotherObject, object);
    ...
    function equalProps(a, b) {
      for (var k in a) {
        if (a.hasOwnProperty(k)) {
          if (!JsHamcrest.areObjectsEqual(a[k], b[k])) {
            return false;
          }
        }
      }
...
JsHamcrest.Matchers.equalTo = function(expected) {
  return new JsHamcrest.SimpleMatcher({
    matches: function(actual) {
      return JsHamcrest.areObjectsEqual(expected, actual);
    },
