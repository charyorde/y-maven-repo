	...
	tinymce.create('tinymce.dom.DOMUtils', {
		...
		split : function(pe, e, re) {
			...
			if (pe && e) {
                ...
                bef = t._extractRangeContents(r);

				// Get after chunk
				r = t.createRng();
				r.setStart(e.parentNode, t.nodeIndex(e) + 1);
				r.setEnd(pe.parentNode, t.nodeIndex(pe) + 1);
                aft = t._extractRangeContents(r);
        ...
        _extractRangeContents: (function() {
                    if (tinymce.isIE9) {
                        return function fakeExtract(rng) {
                            var fakeRng = new tinymce.dom.Range(this);
	...
	tinymce.create('tinymce.dom.Selection', {
		...
		getRng : function getRng(w3c) {
			...
			try {
				...
				if (s && s.rangeCount > 0){
                    r = s.getRangeAt(0);
                }else if (getRng.lastRng){
                    r = getRng.lastRng;
                }else if (s && s.createRange){
                    r = s.createRange();
                }else{
                    r = doc.createRange();
                }
				...
				// IE throws various errors here, for reasons that are not entirely clear
                console.log("failed to get current range; creating a new one instead.", ex);
			}
            ...
            if(r && ((r.getParentElement && r.getParentElement().ownerDocument == doc) || (r.startContainer && r.startContainer.ownerDocument == doc))){
                getRng.lastRng = r;
            }else{
                getRng.lastRng = null;
            }
...
(function(tinymce) {
	...
	tinymce.create('tinymce.Editor', {
		...
		setupIframe : function() {
            ...
            //Focus events
            t.dom.bind(b, "focus", function(){
                t.onFocus.dispatch(t);
            });
            t.dom.bind(b, "blur", function(){
                t.onBlur.dispatch(t);
            });
        ...
        scheduleNodeChanged: function scheduleNodeChanged(){
            if(!scheduleNodeChanged.isScheduled){
                scheduleNodeChanged.isScheduled = true;
                var t = this;
                setTimeout(function(){
                    t.nodeChanged();
                    scheduleNodeChanged.isScheduled = false;
                }, 0);
            }
        },
		...
		_addEvents : function() {
            ...
            //Tapping anywhere inside the editor frame should focus on the editor in mobile safari.
            if(tinymce.isIDevice){
                dom.bind(dom.select("html"), "click", function(){
                    t.focus();
                });
            }
	...
	tinymce.EditorCommands = function(editor) {
		...
		addCommands({
			...
			mceInsertContent : function(command, ui, value) {
				...
				bookmarkHtml = '<span class="mce_marker" data-mce-type="bookmark">\uFEFF</span>';
				...
				if (!args.invalid) {
					...
					value = serializer.serialize(
						parser.parse(
							...
							value.replace(/<span (class="mce_marker"|class=mce_marker).+?<\/span>/i, function() {
				...
				marker = dom.select('.mce_marker')[0];
                if(marker){
...
(function(tinymce) {
	...
	function isAtEnd(rng, par) {
        ...
        var remainder = rng2.toString().replace(/\ufeff/g, "");  //strip off byte order marks, which often occur in IE9
	...
	tinymce.create('tinymce.ForceBlocks', {
                ...
                if (!isIE || isIE9) {
					...
					ed.onKeyUp.add(function(ed, e) {
						...
						if (e.keyCode == 13 && !e.shiftKey) {
                            ...
                            // Press Enter from inside a header, strip header from new line
                            parent = dom.getParent(parent, dom.isBlock);
                            if(/^(H[1-6])$/.test(parent.nodeName) && !parent.innerText) {
                                parent = dom.rename(parent, 'P');
        ...
        insertPara : function(e) {
            ...
            if(blockContainer.nodeName.toLowerCase() == 'li'){
                ...
                //outdent empty list items at the end of the list.  "Empty" includes list items that contain an empty formatting element, and possibly a terminal BR.
                if((dom.isEmpty(li) || (li.firstChild.nodeType == 1 && dom.isEmpty(li.firstChild) && (li.childNodes.length == 1 || li.lastChild.nodeName.toLowerCase() == 'br' && li.childNodes.length == 2))) &&
                        li.nextSibling == null){
	...
	tinymce.create('tinymce.ControlManager', {
		...
		createColorSplitButton : function(id, s, cc) {
			...
			s = extend({
				...
				scope : s.scope,
				more_colors_title : ed.getLang('advanced.more_colors')
			}, s);
		...
		function expandRng(rng, format, remove) {
			...
			function findParentContainer(container, child_name, sibling_name, root) {
				...
				if (container.nodeType == 3 && !isWhiteSpaceNode(container)) {
					if (child_name === 'firstChild' ? startOffset > 0 : endOffset <= container.nodeValue.length) {
						return container;
					}
				}
...
(function() {
    ...
    //                this.logRange("Expanded block range: ", ret);
            ...
            normalizeRange: function(rng) {
                ...
                function isNodeType(n, elementName) {
                    elementName = elementName.toLowerCase();

                    return n.nodeName.toLowerCase() == elementName;
                }
                ...
                function isAfterFormattingElement(treePos){
                    return treePos.off > 0 && treePos.c.childNodes[treePos.off-1].nodeType == 1
                        && !isBlockNode(treePos.c.childNodes[treePos.off-1])
                        && !isNodeType(treePos.c.childNodes[treePos.off-1], "img")
                        && !isNodeType(treePos.c.childNodes[treePos.off-1], "br");
                }

                function findLastFormattingElement(treePos){
                    var n = treePos.c.childNodes[treePos.off-1];
                    while(n.lastChild) {
                        n = n.lastChild;
                    }
                    return {c: n, off: n.childNodes.length}
                }
                ...
                function adjustCursorPosition(c, off) {
                    ...
                    if (treePos) {
                        ...
                        if (treePos && (treePos.c != c || treePos.off != off)) {
                            return treePos;
                        }

                        if(isAfterFormattingElement({c: c, off: off })) {
                            return findLastFormattingElement({c: c, off: off });
                        }
                        return {c: c, off: off };
                    }
                }
