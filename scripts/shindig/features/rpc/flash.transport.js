...
if (!gadgets.rpctx.flash) {  // make lib resilient to double-inclusion
  ...
  gadgets.rpctx.flash = function() {
    ...
    // [HS] flush pendingHandshake check
    var PENDING_HANDSHAKE_TIMEOUT_MS = 500;
    var pendingHandshakesHandle = null;
    ...
    function flushHandshakes() {
      if (relayHandle !== null && relayHandle['setup']) {
        ...
        // [HS] Clear flushHandshakes timeout
        if (pendingHandshakesHandle !== null) {
          window.clearTimeout(pendingHandshakesHandle);
          pendingHandshakesHandle = null;
        }
      } else if (pendingHandshakesHandle === null && pendingHandshakes.length > 0) {
        // [HS] Add timeout for checking pending handshake
        pendingHandshakesHandle = window.setTimeout(flushHandshakes, PENDING_HANDSHAKE_TIMEOUT_MS);
      }
    }
    ...
    return {
      ...
      setup: function(receiverId, token) {
        ...
        // [HS] try to flush pending handshake for more than one applications in a page.
        flushHandshakes();
