...
gadgets.io = function() {
  ...
  // [HS] Object to store ids for the ajax poll to avoid memory leak
  var ajaxPollQ = {};
  ...
  function makeXhrRequest(realUrl, proxyUrl, callback, paramData, method,
    ...
    if (callback) {
        var closureCallback = gadgets.util.makeClosure(null, processResponseFunction,
            realUrl, callback, params, xhr);

      // [HS] add gate for alternate ajax state change event handler
      var shouldPoll = shouldPollReadyStateChange();
      if(shouldPoll) {
        // [HS] Replace the hook for onreadystatenchange handler
  ...
  /**
   * [HS]
   * Helper function to check if need to poll for Ajax readystate change.
   * Default will check for IE browsers with active x supported.
   * @return true if we need to add polling to handle ready state change and false otherwise.
   */
  function shouldPollReadyStateChange() {
  ...
  /**
   * [HS]
   * Helper function to set interval to call the callback for Ajax to avoid memory leak in IE7.
   *
    ...
    ajaxPollQ[tempTid] = window.setInterval(
      function() {
        if(xhr && xhr.readyState === 4) {
          // Clear the polling interval for the transaction and remove the reference from ajaxPollQ
