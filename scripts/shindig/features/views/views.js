...
/**
 * TODO: [HS] temporary override until we merge fix from Shindig to fix SHINDIG-1686.
 */
...
gadgets.views = function() {
  ...
  function init(config) {
      ...
      if (conf.hasOwnProperty(s)) {
        ...
        if (s != 'rewriteLinks') {
          ...
          for (var i = 0, alias; (alias = aliases[i]); ++i) {
            if (!supportedViews.hasOwnProperty(alias)) {
    ...
    // [HS] Fix for JIVE-12010
    var viewName = urlParams['view'] || "";
    if(viewName.split(".").length > 1) {
      var viewNameMajor = viewName.split(".")[0];
