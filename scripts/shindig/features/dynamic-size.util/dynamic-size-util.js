...
(function() {
  ...
  function parseIntFromElemPxAttribute(elem, attr) {
   ...
   /**
   * [HS] Helper function to check if element should be event processed.
   * Default implementation will ignore the element with position other than static.
   *
   * @private
   * @param {Element} elem the element to be checked.
   */
  function shouldProcessElement(elem) {
    if(typeof elem.style === 'undefined') {
      return false;
    }

    var position = elem.style.position || getElementComputedStyle(elem, 'position');
      ...
      for (var i = 0; i < children.length; i++) {
        ...
        if (typeof child.style != 'undefined') {  // Don't measure text nodes
          ...
          if (typeof start != 'undefined' && typeof size != 'undefined') {
            ...
            var endMargin = (parseIntFromElemPxAttribute(child, dimenEnd) || 0);
            var end = start + size + endMargin;

            // [HS] check if we need to include the dimension of the child to the final result
