...
$.fn.media.swf = function(el, opts) {
	...
	if ($.fn.flash) {
		...
		// insert swfobject content in $div
		$div.flash($.extend({
			swf: opts.src,
			id: el.id,
			width: opts.width,
			height: opts.height,
			hasVersion: opts.flashVersion,
			expressInstall: opts.expressInstaller,
			flashvars: opts.flashvars
		}, opts.params));
		$div.find('object').attr(opts.attrs);
	}
	// swfobject < v2
	else {
		$(el).after($div).remove();
