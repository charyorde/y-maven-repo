...
(function($)
{
	...
	function heightTo(self)
	{
		...
		var overflow = _self.css('overflow');
		_self.stop();
		if(_self.height() < _self.data("_vgwrapheight"))
		{
			...
			else
			{
				...
				// jQuery#animate() automatically sets overflow to "hidden"
				_self.css('overflow', overflow);
			}
		}
		else
		{
			...
			_self.data("_vgwraptimeout", setTimeout(function(){
				if($.browser.msie)
				{
					// Check to make sure that _self has not been removed from the
					// DOM or anything like that.
					if (_self.data("_vgwrapheight")) {
						_self.height(_self.data("_vgwrapheight"));
					}
				}
				else
	...
	$.fn.extend({
		vgrid: function(option)
		{
			...
			_self.data('_vgresizehandler', function(e)
			{
				var $window = $(window);
				...
				// Check to make sure that _self has not been removed from the
				// DOM by verifying that it still has data attributes.
				// Also check that the window width or height has
				// actually changed to work around an IE issue where the
				// resize event fires all the frigging time.
				if (_self.data("_vgchild") &&
			...
			$window.resize(_self.data('_vgresizehandler'));
		...
		{
			var _self = $(this);
			$(window).unbind('resize', _self.data('_vgresizehandler'));
			clearTimeout('_vgwraptimeout');
		}
	});
})(jQuery);
