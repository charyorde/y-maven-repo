...
$.widget("ui.multidraggable", $.extend({}, $.ui.draggable.prototype, {
	_create: function() {
		...
		if (this.options.helper == 'original' && !(/^(?:r|a|f)/).test(this.element.css("position"))) {
            ...
            this.options.origHelper = this.options.helper;
        }
    ...
    _mouseUp: function(event) {
		...
		if (this.element.hasClass('ui-multidraggable') && !this.element.hasClass('push')) {
			this.element.removeClass('ui-multidraggable');
		}
        else {
            $j('li.ui-multidraggable.push').removeClass('push');
        }
        return true;
	},
	...
	_mouseCapture: function(event) {
        ...
        if (!this.element.hasClass('ui-multidraggable')) {
            this.element.addClass('ui-multidraggable push');
        }
	...
	mouseStart: function(event, draggedOptions) {
		...
		var o = this.options;
        if (!this.element.hasClass('push')) {
            this.element.addClass('push');
        }

		this.options.draggedOptions = draggedOptions;

        //Create and append the visible helper
        if ($.isFunction(this.options.helper)) {

            if (!event.createdFunctionHelper) {
                event.createdFunctionHelper = true;
            }
            else {
                this.options.helper = function() {return "<div></div>";};
            }
        }
	...
	mouseStop: function(event) {
		...
		var dropped = false;
		if ($.ui.ddmanager && !this.options.dropBehaviour && !event.multiDropProcessed) {
            event.multiDropProcessed = true;
	...
	_clear: function() {
        ...
        this.options.helper = this.options.origHelper;
	},
		...
		$.each(inst.sortables, function(i) {
			...
			if(this.instance._intersectsWith(this.instance.containerCache)) {
				...
				if(!this.instance.isOver) {
					...
					this.instance._mouseUp(event, true);    //??is this correct?
