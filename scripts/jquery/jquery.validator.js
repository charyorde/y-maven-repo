...
(function($) {
    ...
    // globals
    var typeRe = /\[type=([a-z]+)\]/, numRe = /^-?[0-9]*(\.[0-9]+)?$/, dateInput = $.tools
        .dateinput, // http://net.tutsplus.com/tutorials/other/8-regular-expressions-you-should-know/
        emailRe = /^([a-z0-9_\.\-\+]+)@([\da-z\.\-]+)\.([a-z\.]{2,6})$/i, urlRe = /^(https?:\/\/)?[\da-z\.\-]+\.[a-z\.]{2,6}[#&+_\?\/\w \.\-=]*$/i, v;
    ...
    function getPosition(trigger, el, conf) {
        ...
        var top = trigger.offset().top, left = trigger.offset().left, pos = conf.position
            .split(/,?\s+/), y = pos[0], x = pos[1];
    ...
    v.fn("[required]", "Please complete this mandatory field.", function(el, v) {
        ...
        //Do not trim whitespace on file input fields
        if(!el.is("input:file")){
            // trim the white space
            el.val($j.trim((el.val())));
        }
        // if same value as the placeholder, fail
    ...
    function Validator(inputs, form, conf) {
        ...
        // private variables
        var self = this, fire = form.add(self);
        ...
        $.extend(self, {
            ...
            reflow: function() {
                inputs.each(function() {
                    var input = $(this), msg = input.data("msg.el");
...
//{{{  checkValidity() - flesh and bone of this tool
            ...
            checkValidity: function(els, e) {
                ...
                els.not(":radio:not(:checked)").each(function() {
                    ...
                    var msgs = [], el = $(this).data("messages", msgs), event = dateInput &&
                        el.is(":date") ? "onHide.v" : conf.errorInputEvent + ".v";
