...
jive.conc.Promise = function() {
     ...
     * return `undefined`.
     *
     * @param {number}  [timeout] time in milliseconds to wait before aborting the promise
 ...
 */
jive.conc.Promise.prototype.then = function(fulfilledHandler, errorHandler) {
...
/**
 * Given a function, returns a new promise that returns the successful
 * outcome of this promise after being transformed by the given
 * function.
 *
 * @param {Function} f transforms success value of this promise
 * @returns {jive.conc.Promise} returns a new promise
 */
jive.conc.Promise.prototype.map = function(f) {
    return this.then(f);
};

/**
 * Given a function that takes the eventual value of this promise,
 * returns a new promise.  The function should return a third promise.
 * The new promise that is returned by this function will result in the
 * same outcome as the third promise.
 *
 * @param {Function} f maps value of this promise to a new promise
 * @returns {jive.conc.Promise} returns a new promise
 */
jive.conc.Promise.prototype.flatMap = function(f) {
    var promise = new jive.conc.Promise();

    this
    .addCallback(function(/* arguments.. */) {
        promise.proxy(f.apply(null, arguments));
    })
    .addErrback(function(/* arguments.. */) {
...
jive.conc.Promise.prototype.proxy = function(promise) {
    var self = this;

    promise
    .addCallback(function() {
        self.emitSuccess.apply(self, arguments);
    })
    .addErrback(function() {
        self.emitError.apply(self, arguments);
    });
};

/**
 * Returns a new jQuery Deferred promise that is backed by this Jive promise.  API transition mechanism.
 * @return {jQuery.Deferred}
 */
jive.conc.Promise.prototype.deferred = function(){
